...
func postIsu(c echo.Context) error {
	...
	if useDefaultImage {
		...
		err = ioutil.WriteFile("../../images/"+jiaIsuUUID, image, 0755)
		if err != nil {
			return c.NoContent(http.StatusInternalServerError)
		}
	}
...
func getIsuConditions(c echo.Context) error {
	...
	conditionLevels, err := calculateConditionLevelValueFromConditionLevelNames(conditionLevelCSV)
	if err != nil {
...
func calculateTrendRes(c echo.Context) ([]TrendResponse, error) {
	...
	for _, character := range characterList {
		...
		// err = db.Select(&conditions, "select isu.id as isu_id,isu_condition.condition_level,isu_condition.timestamp from isu_condition join isu on isu.jia_isu_uuid=isu_condition.jia_isu_uuid where isu_condition.id in (select max(isu_condition.id) from isu_condition join isu on isu.jia_isu_uuid=isu_condition.jia_isu_uuid where isu.character=? group by isu_condition.jia_isu_uuid) order by timestamp desc", character.Character)
		// if err != nil {
		// 	c.Logger().Errorf("db error: %v", err)
		// 	return nil, c.NoContent(http.StatusInternalServerError)
		// }
		...
		for _, isu := range isuList {
			...
			err = db.Select(&conditions,
				"SELECT `condition_level`, `timestamp` FROM `isu_condition` WHERE `jia_isu_uuid` = ? ORDER BY timestamp DESC LIMIT 1", // 最新一件で良いはず
				isu.JIAIsuUUID,
			)
			if err != nil {
				c.Logger().Errorf("db error: %v", err)
...
// コンディションからレベル数値を計算
// "is_dirty=true,is_overweight=false,is_broken=true" -> 1
func calculateConditionLevelValueFromConditionStr(conditionStr string) (int, error) {
	var conditionLevel int

	warnCount := strings.Count(conditionStr, "=true")
	switch warnCount {
	case 0:
		conditionLevel = 0
	case 1, 2:
...
// コンディションレベルの文字列からレベル数値を計算
// "warning,info" -> [0, 1]
func calculateConditionLevelValueFromConditionLevelNames(condition string) ([]int, error) {
	var result []int
...
func calculateConditionLevelStrFromConditionLevelValue(conditionLevel int) (string, error) {
	var conditionLevelStr string

	switch conditionLevel {
	case 0:
		conditionLevelStr = conditionLevelInfo
	case 1:
		conditionLevelStr = conditionLevelWarning
	case 2:
		conditionLevelStr = conditionLevelCritical
	default:
